...
from setup.pages.annahmen_new import annahmen_new
...
annahmen_new_graph_and_tables_list = []
for p in annahmen_new:
    logger.info(f"Creating page {p.title}")
    lst = utils.create_annahmen_new(p,synopse,settings)
    annahmen_new_graph_and_tables_list.append(lst)
...
for p in annahmen_new_graph_and_tables_list:
...
template = env.get_template('slide_template_annahmen.html')
